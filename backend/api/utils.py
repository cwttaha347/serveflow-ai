...
def calculate_match_score(request_obj, provider):
    ...
    if provider.rating is not None:
        rating_val = float(provider.rating)
        rating_score = (rating_val / 5.0) * 35
        score += rating_score
    ...
    # 6. SPECIALIST BOOST (Truly Dynamic - 0-35 points)
    # This ensures that an "Electrical Expert" beats a "Plumbing Expert" for Electrical jobs
    try:
        from .models import Category
        cat = Category.objects.get(id=req_category_id)
        cat_name = cat.name.lower()
        
        # Primary Identity Check: Does their name or bio start with the category?
        provider_full_name = (provider.user.get_full_name() or provider.user.username).lower()
        bio_lower = (provider.bio or "").lower()
        skills_text = " ".join(provider.skills or []).lower()
        
        relevance_score = 0
        
        # 1. Direct Category Match in Name/Bio (Strongest Signal)
        if cat_name in provider_full_name or cat_name in bio_lower:
            relevance_score += 25
            
        # 2. Skill/Context Match (Secondary Signal)
        # Check title keywords against provider skills/bio
        job_title = (request_obj.get('title') if isinstance(request_obj, dict) else request_obj.title or '').lower()
        if job_title:
            title_words = [w for w in job_title.split() if len(w) > 3]
            for word in title_words:
                if word in skills_text or word in bio_lower:
                    relevance_score += 10
                    break # Max 10 for title words
                    
        # 3. Category Name in Skills
        if cat_name in skills_text:
            relevance_score += 15
            
        score += min(35, relevance_score) # Cap boost at 35
    except:
        pass
