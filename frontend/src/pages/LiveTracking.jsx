    ...
    return (
        <div className="h-[calc(100vh-100px)] flex flex-col lg:flex-row gap-6">
                            ...
                            return (
                                <div key={step.status} className={`relative flex items-center gap-4 ${isPast ? 'opacity-100' : 'opacity-40'}`}>
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center z-10 bg-white dark:bg-slate-800 
                                        ${isActive ? 'border-blue-600 text-blue-600 animate-pulse' : isPast ? 'border-green-500 text-green-500' : 'border-slate-300 text-slate-300'}`}>
                                        <step.icon className="w-3 h-3" />
                                    </div>
                                    <span className={`font-medium ${isActive ? 'text-blue-600' : 'text-slate-600 dark:text-slate-400'}`}>
                                        {step.label}
                                    </span>
                                </div>
                            );
            ...
            <div className="flex-1 bg-slate-200 dark:bg-slate-700 rounded-xl relative overflow-hidden group">
                ...
                <div
                    className="absolute w-4 h-4 bg-blue-600 rounded-full border-2 border-white shadow-lg transition-all duration-1000 ease-linear"
                    style={{
                        top: '50%',
                        left: '50%',
