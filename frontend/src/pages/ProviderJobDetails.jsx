...
const ProviderJobDetails = () => {
    ...
    return (
                        ...
                        <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
                            ...
                            {['accepted', 'started', 'completed'].includes(job.status) && (
                                <button
                                    ...
                                    className="w-full mt-3 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 font-medium"
                                >
                                    <MessageSquare className="w-4 h-4" /> Message Customer
                                </button>
                            )}
                            {job.status === 'pending' && (
                                <button className="w-full mt-3 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 rounded-lg flex items-center justify-center gap-2 font-medium cursor-not-allowed" disabled>
                                    <MessageSquare className="w-4 h-4" /> Accept to Chat
                                </button>
                            )}
                        </div>
                    </div>
        ...
        {activeChat && (
            <ChatInterface
                jobId={Number(id)}
                otherUser={{ username: job.request?.user?.first_name || job.request?.user?.username || 'Customer' }}
                isOpen={activeChat}
