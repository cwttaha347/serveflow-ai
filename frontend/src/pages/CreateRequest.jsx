...
const CreateRequest = () => {
    ...
    const fetchSnapshot = async () => {
        setFetchingRecs(true);
        try {
            const res = await api.post('requests/flow/snapshot/', {
            ...
            setRecommendations(res.data.providers || []);
        } catch (err) {
            console.error(err);
            showError(err.response?.data?.error || 'Failed to analyze request.');
                    ...
                    {step === 3 && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-10 duration-500">
                            <div className="text-center mb-8">
                                ...
                                <p className="text-slate-400">Set timing, then AI computes budget and hours.</p>
                            </div>
                    ...
                    {step === 4 && (
                        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
                            <div>
                                ...
                                <p className="text-slate-400">Matched providers ranked by AI scoring based on skills, ratings, and past job history.</p>
                            </div>
                            {snapshot?.analysis && (
                                <div className="grid md:grid-cols-3 gap-4">
                                    <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-700">
                                        <p className="text-xs text-slate-400 uppercase">Estimated Hours</p>
                                        <p className="text-xl font-black text-blue-300">{snapshot.analysis.estimated_hours}</p>
                                    </div>
                                    <div className="p-4 rounded-xl bg-slate-900/70 border border-slate-700">
                            ...
                            {fetchingRecs ? (
                                <div className="flex flex-col items-center justify-center py-12 space-y-4">
                                    ...
                                    <p className="text-blue-400 font-bold animate-pulse">Matching providers...</p>
                                </div>
                            ) : recommendations.length > 0 ? (
                                <div className="grid gap-6">
                                    {recommendations.map((prov, index) => (
                                        <div key={prov.provider_id || prov.id} className="group relative">
                                            ...
                                            <div className="relative p-6 bg-slate-900 border border-slate-700/50 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-6 overflow-hidden">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-2xl font-black shrink-0">
                                                        {prov.user?.first_name?.[0] || prov.provider_name?.[0] || 'P'}
                                                    </div>
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <h3 className="text-xl font-bold">{prov.user?.first_name || prov.provider_name} {prov.user?.last_name || ''}</h3>
                                                        ...
                                                        <div className="flex items-center gap-4 text-sm">
                                                            ...
                                                            <span className="text-slate-400">{prov.experience_years || 0} Yrs Exp.</span>
                                                            <span className="text-slate-500">•</span>
                                                            <span className="text-slate-400">{prov.completed_jobs || 0} Jobs Done</span>
                                                        </div>
                                                        <div className="mt-2 flex flex-wrap gap-2">
                                                            ...
                                                            {(prov.skills || []).slice(0, 3).map((skill, i) => (
                                                                <span key={i} className="px-2 py-1 bg-purple-500/10 text-purple-400 text-[10px] font-bold rounded-md border border-purple-500/20">
                                                                    {skill}
                                                                </span>
                                                            ))}
                                                        </div>
                                                ...
                                                <button
                                                    onClick={() => handleSubmit('manual', prov.provider_id || prov.id)}
                            ...
                            <div className="pt-6">
                                <div className="grid md:grid-cols-2 gap-4">
                                    <button
                                        onClick={() => handleSubmit('auto', null)}
                                        className="w-full py-4 bg-blue-700 text-white rounded-xl font-bold hover:bg-blue-600 transition-colors border border-blue-500"
                                    >
                                        Auto Mode (AI Assign)
                                    </button>
                                    <button
                                        onClick={() => handleSubmit('broadcast', null)}
