            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="w-full sm:w-auto px-6 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-all shadow-sm"
                    >
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {providers.filter(p => p.verification_status !== 'verified').length === 0 ? (
                                ...
                                providers.filter(p => p.verification_status !== 'verified').map((provider) => (
                                    <div key={provider.id} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all">
                                        <div className="flex items-center gap-4 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <h3 className="font-black text-slate-900 dark:text-white truncate">Provider Unit #{provider.id}</h3>
                                        ...
                                        <div className="flex flex-wrap gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                className="px-4 sm:px-6 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 hover:scale-105 active:scale-95 transition-all"
                                            >
                                                Authorize
                                            </button>
                                            <button
                                                onClick={() => navigate('/dashboard/admin/providers')}
                                                className="px-4 sm:px-6 py-2 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all"
                                            >
                                                Audit
                                            </button>
                                        </div>
